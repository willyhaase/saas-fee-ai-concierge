...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "category, intent, guest_message, detected_restaurants, detected_activities, detected_entities, created_at"
      )
    ...
    return Response.json(buildStats((data ?? []) as AnalyticsRow[], days));
  } catch (error) {
    const message =
